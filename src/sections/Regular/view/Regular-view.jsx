...
import FormControlLabel from '@mui/material/FormControlLabel'; // Added import
...
export default function RegularView() {
  ...
  const [formData, setFormData] = useState({
    ...
    visit_9: '',
    visit_10: '',
    bloodbank_conf: false, // Initialized as false
    insurance_conf: false, // Initialized as false
  });
  ...
  const resetFormData = () => {
    setFormData({
      ...
      visit_9: '',
      visit_10: '',
      bloodbank_conf: false, // Reset to false
      insurance_conf: false, // Reset to false
    });
  ...
  const handleSubmit = async () => {
    try {
      ...
      const { data, error } = await supabase.from('clinic').insert([cleanedFormData]);
      if (error) {
        throw error;
      }
      handleCloseModal();
                ...
                {filteredUsers
                  ...
                  .map((row) => (
                    <UserTableRow
                      ...
                      bloodbank_conf={row.bloodbank_conf ? 'Yes' : 'No'} // Display 'Yes' or 'No'
                      insurance_conf={row.insurance_conf ? 'Yes' : 'No'} // Display 'Yes' or 'No'
      ...
      <Modal open={openModal} onClose={handleCloseModal}>
        ...
        >
          ...
          >
            ...
            {/* Added checkboxes for bloodbank_conf and insurance_conf */}
            <FormControlLabel
              control={
                <Checkbox
                  name="bloodbank_conf"
